...
## 4-b. 시뮬레이터 표기 규약 (NFR6 · 2026-07-22 리뷰)

가전 시뮬레이터(`appliance_sim/`)의 "시뮬레이터 — 실가전 아님" 배너에 적용한다.
캐리어 어댑터의 정직 표기(`is_device=False`, "참조 어댑터 — 실기기 아님")와 같은 계보.

**규칙: 경계마다 한 번, 반복 스트림 안에서는 생략.**

| 경로 | 배너 | 이유 |
|---|---|---|
| 화면 — 기동 헤더·블록 헤더·종료 푸터 | ✅ 1회씩 | 경계에서 맥락을 세운다 |
| 화면 — 반복 줄(명령 반영·상태 전이) | ❌ | 반복 문구는 시각적 노이즈가 되어 필터링된다 |
| 화면 — 오류·거부 줄 | ✅ | 예외적 사건이라 노이즈가 아니라 신호다 |
| 자료구조 — 이벤트·상태 페이로드 | ✅ 항상 | 기계가 읽으므로 노이즈 문제 없음 |
| BLE 광고명 (`SIM-NOT-REAL-<ref>`) | ✅ | 스캐너에선 한 번만 보인다 — 맥락이 다르다 |

**왜 개수를 세는 테스트로는 부족한가.** 초판은 12줄 중 9줄에 배너를 붙였고
테스트는 `out.count(BANNER) >= 6`으로 통과했다. 그 단언이 재는 것은 문자열
개수이지 **표기가 읽히는지**가 아니다 — 아홉 번 붙이면 아홉 번 안 읽힌다.
표기를 빼자는 것이 아니라 표기가 **작동하게** 하자는 것이며,
회귀 테스트도 "경계마다 정확히 있고 스트림엔 없다"를 단언하도록 바꿨다
(`test_banner_appears_at_every_boundary_not_every_line`).
1.1의 "'단어 언급' 단언 금지" 교훈이 표기 영역에서 재발한 사례다.
