...
| 예산 상수의 어댑터 이관 (1.3) | `BUDGET_*`·`BLE_MTU`는 **가민 값**이므로 가민 어댑터가 `capabilities()`로 신고. 단 `storage.py` 상수는 1.2 테스트 호환을 위해 **삭제하지 않고 유지** — 어댑터가 그 값을 참조해 신고한다 | 코어 상수로 남기면 캐리어 중립이 이름뿐이 된다. 상수를 즉시 삭제하면 1.2 회귀가 깨진다(스토리 1.3 규약). 값의 단일 출처는 storage.py, 캐리어별 의미 부여는 어댑터 |
...
## 5.5 파티 결정: 보수 예산 + 기기 단위 분할 (2026-07-22)

§5.4에서 "현재 상한으로는 섹션 분할로도 어떤 예산에도 안 들어간다"로 끝났던 문제를
파티에서 다음으로 해소했다.
...
| # | 문제 | 결정 | 근거 |
|---|---|---|---|
| 1 | 예산 상수 | **보수 4,096B 단일 기준** | 포럼 8KB의 절반. 값 자체가 마진 |
| 2 | 개수 상한 | **현재값 유지 = 보안 천장** | Night Keeper 풀가전 페르소나를 안 자른다 |
| 3 | 저장 전략 | **기기 단위 분할** | 조각당 예산으로 문제를 나눈다 |
| 4 | MARGIN | **폐기 유지** | 보수값이 곧 마진, 이중 적용 금지 |

### 왜 기기 단위 분할이 셋을 동시에 푸는가 (실측)

섹션 통째로는 routines가 최악 46,381B였다. 하지만 저장 키를
**meta / device:&lt;ref&gt; / routine:&lt;i&gt;** 로 쪼개면 —

| 조각 (최악 케이스) | 바이트 | 보수 4,096B |
|---|---|---|
| 기기 1대 (설정 포함) | 1,772 B | ✅ |
| 루틴 1개 | 2,328 B | ✅ |
| meta (기기30 인덱스) | 1,134 B | ✅ |

**보안 천장(기기30·루틴20)의 최악 케이스에서도 모든 조각이 보수 예산에 들어간다.**
총량도 99,720B / 131,072B로 예산 내. 이 주장은 테스트로 고정했다
(`test_worst_case_ceiling_fits_per_key_budget`).

데모 규모(가민+애플워치 2대, 가전 몇 대)는 기기 1대당 154B로 예산의 4%.

### 코드 반영

- `MARGIN`·`BUDGET_REFERENCES`·`SECTION_KEYS` 삭제 → `BUDGET_PER_KEY = 4096` 단일값
- `split_chunks(profile)` 신설 — 저장 키 경계 정의 (전송 청킹 20B는 Epic 2가 상속)
- `size_report`가 조각 단위 판정(`per_key_ok`·`largest_chunk`)으로 재작성

### 실기기 실측 항목에 추가 (§5.4 계획 보강)

- [ ] **Storage 키 개수 상한 확인** — 최악 시 51개 키(기기30+루틴20+meta). CIQ의
      "16 슬롯" 제한은 앱·위젯·워치페이스 슬롯이지 Storage 키 개수가 아니라고
      조사됐으나(§5.3), Storage 키 개수 제한이 없다는 명시적 근거도 없다.
      `setValue`를 키 개수를 늘리며 반복해 한계를 실측할 것.
