# ThinQ OnMe — 워치 앱 개발 킥오프 (새 창 인계용)

작성 2026-07-22 · 이 문서는 **새 세션이 콜드 스타트로 워치 앱 개발을 시작**하기 위한 인계 노트다.

> **용어 정정 (2026-07-31)**: 작성 당시 "프로토/프로토타입"으로 부르던 현재 구현물은
> 표준 정의상 **PoC**다(실사용자 0명·미출시·가전은 시뮬레이터). 아래 "새 창에서 할 일"의
> 워치 앱 트랙이 **Prototype 단계 진입**에 해당한다.
> 단계 라벨 단일 출처: [MATURITY_POSITION.md](MATURITY_POSITION.md).

---

## 지금 무엇을 만드는가

**ThinQ OnMe** — "집이 나를 따라온다(Home Follows You)"의 PoC 구현.
...
### 실기기 실측 항목 (가민 보유 — Epic 4 백로그였으나 워치 앱에 필요)
