...
- [ ] **Task 1: 캐리어 인터페이스 정의** (AC: 1, 3)
  - [ ] `home_profile/carrier.py` 신설 — 표준 라이브러리만(1.1·1.2 의존성 원칙 승계).
        `typing.Protocol` 또는 순수 클래스로 정의하되 **런타임 의존성 0건**
  - [ ] 인터페이스는 **불투명 바이트 레코드 맵**을 다룬다:
  ...
  - [ ] `CarrierCapabilities` — 어댑터가 **자기 한계를 스스로 신고**하는 값 객체:
  ...
  - [ ] `CarrierStatus` — `SUPPORTED` / `UNIMPLEMENTED` 2값. 3값 이상으로 늘리지 말 것
        (상태를 늘리면 "부분 동작"이라는 회색지대가 생기고 그게 AC4가 막는 것이다)

- [ ] **Task 2: 참조 어댑터 1개 — 실제로 도는 것** (AC: 1, 3)
  - [ ] `MemoryCarrier`(또는 `LocalFileCarrier`) **1종만** 구현. 이것이 "어댑터 모양이
        실재한다"는 유일한 증거이며, Epic 2에서 호스트 측 시연에도 쓰인다
  - [ ] 자기 표기: 이 어댑터는 **워치가 아니다**. `is_device=False` + 라벨
        `"참조 어댑터 — 실기기 아님"`. 1.2의 시뮬레이터 표기 규약(NFR6)과 동일 계보
  - [ ] 용량 한계를 **실제로 강제**한다 — `max_record_bytes` 초과 시 거부.
        한계를 신고만 하고 통과시키면 어댑터가 거짓말을 하는 것이고,
        가민에서만 터지는 코드가 테스트를 통과한다

- [ ] **Task 3: 가민 어댑터 — 미구현 정직 표기** (AC: 4)
  - [ ] `GarminConnectIQCarrier` — `status = UNIMPLEMENTED`.
        모든 연산은 **예외 없이** `["미구현: Garmin Connect IQ 어댑터는 Monkey C
        런타임을 요구하며 이 저장소에 구현체가 없다"]`를 반환한다
  - [ ] ⚠️ **이 저장소에 벤더 SDK가 없고 설치할 수도 없다** — Connect IQ SDK는
        Monkey C이며 Python 패키지가 아니다. 그러므로 이 어댑터는 **경계 설계와
        용량 신고까지만** 담당한다. "곧 되는 것처럼" 보이는 코드를 쓰지 말 것
  - [ ] `capabilities()`는 신고하되 **출처 라벨을 반드시 포럼/미확인으로** 남긴다
        (128KB·8KB·20B는 공식 문서 보증이 아니다 — PROFILE_SCHEMA.md §5)
  - [ ] `zlib` 해제 지원 여부는 **미확인**으로 신고한다(`supports_decompression=None`).
        `False`도 `True`도 아니다 — 모르는 것을 아는 것으로 세탁하지 않는다

- [ ] **Task 4: 애플·샤오미는 만들지 말 것** (AC: 3, 4)
  - [ ] 빈 어댑터·빈 레지스트리를 **생성하지 않는다**. AC3이 요구하는 것은
        "코어 변경 없이 추가 가능함이 **인터페이스 문서로** 제시"이지 스텁 클래스가
        아니다. 1.1에서 빈 `MIGRATIONS` 레지스트리가 삭제됐다 — 같은 실수 금지
  - [ ] 대신 `docs/CARRIER_INTERFACE.md`에 **"새 캐리어를 추가하려면"** 절을 쓴다:
        구현해야 할 메서드 목록, 신고해야 할 capabilities, 코어를 **한 줄도** 고치지
        않는다는 서술, 그리고 애플·샤오미 현재 상태 = **미구현**임을 표로 명시

- [ ] **Task 5: 경계 위반 탐지 테스트** (AC: 2)
  - [ ] `tests/test_carrier_neutrality.py` 신설
  - [ ] **코어 모듈 소스를 실제로 읽어** 벤더 import가 없음을 단언한다:
  ...
  - [ ] 문자열 grep이 아니라 **AST 기반**일 것 — 주석·docstring의 "Garmin" 언급은
        위반이 아니다(1.1·1.2 docstring에 이미 등장한다). grep으로 짜면 지금 당장
        빨간불이고, 그걸 피하려 docstring을 지우는 건 문서를 태워 테스트를 맞추는 짓이다
  - [ ] 어댑터 모듈은 이 검사에서 **면제 대상**임을 테스트가 명시(그게 경계의 정의)
  - [ ] 코어에 벤더 import를 **주입한 가짜 소스**로 검사기가 실제로 실패함을 확인
        (검사기 자체의 스텁 판별 — 아래 함정 2)

- [ ] **Task 6: 어댑터 계약 테스트 (공통 스위트)** (AC: 1, 3, 4)
  - [ ] 어떤 어댑터든 통과해야 하는 계약 테스트를 **한 벌** 작성해
  ...
  - [ ] 왕복: 참조 어댑터에 `serialize()` 결과를 넣고 꺼내 `deserialize()` →
        `validate_profile() == []` (1.2 자산 재사용, 새 직렬화 경로 만들지 말 것)
  - [ ] 오류 문구에 **레코드 페이로드 값을 넣지 않는다** — 크기·이름·개수만.
        1.1 리뷰에서 거부 메시지가 PII 값을 그대로 흘린 것이 지적됐다

- [ ] **Task 7: 인터페이스 문서** (AC: 3, 4)
  - [ ] `docs/CARRIER_INTERFACE.md` 신설 — 메서드 계약표, 반환 규약(예외 금지),
        capabilities 필드와 출처 라벨 규칙, "새 캐리어 추가 절차", **캐리어 현황표**
  - [ ] 캐리어 현황표는 상태를 정직하게: 참조 = 동작(실기기 아님),
        가민 = **미구현(경계·용량 신고만)**, 애플 = 미구현, 샤오미 = 미구현.
        발표 자료에 이 표를 그대로 쓸 수 있어야 한다
  - [ ] `docs/PROFILE_SCHEMA.md` §6 표에 이번 설계 판단(예: 예산 상수의 어댑터 이관)
