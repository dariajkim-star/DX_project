...
| 날짜 | 변경 |
|---|---|
| 2026-07-23 | **changes-requested 해소 → done.** 2차 평결의 지적("측정 대상이 합성 픽스처였다, 스키마 최대로 재라")을 현재 코드로 재검증: 스키마 상한 최악 케이스(기기30·cap12·설정값64자·루틴20·액션10)에서 **최대 조각 1,880B < 4,096B, 총량 71,667B < 131,072B**로 통과. 평결 직후 도입된 기기 단위 분할(`split_chunks`)+개수 상한이 원인을 제거했고, `test_worst_case_ceiling_fits_per_key_budget`가 schema 상수에서 파생돼 이를 회귀로 고정하고 있다(상한이 바뀌면 자동 추종). 상태값 갱신만 누락됐던 것으로 확인 — 추가 코드 변경 없음 |
| 2026-07-22 | **파티 재결정: 보수 4,096B 단일 기준 + 기기 단위 분할.** §5.4의 "어떤 예산에도 미달"을 해소 — device/routine 단위로 쪼개면 최악 케이스 조각도 보수 예산 내(기기 1,772B·루틴 2,328B). 개수 상한은 보안 천장으로 유지, MARGIN 폐기. split_chunks 신설, size_report 조각 판정으로 재작성. 129 passed. 상세 §5.5 |
