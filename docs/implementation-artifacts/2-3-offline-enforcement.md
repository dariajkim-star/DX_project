...
- [x] **Task 1: R5 해소 — 전송 계층이 청크를 받는다** (AC: 1)
  - [x] ⚠️ **2.2 파티 리뷰 R5의 이월분이며 이 스토리의 선행 조건이다.**
...
- [x] **Task 2: 오프라인 강제 하네스 — 실행 중 네트워크를 실제로 막는다** (AC: 1)
  ...
  - [x] 차단 시 `OfflineViolation` 예외를 던진다 — 여기서는 **예외가 옳다.**
        이 프로젝트의 예외 금지는 *제품 코드의 계약*이고, 하네스는 시험 장비다.
        위반이 조용히 오류 목록으로 흡수되면 그게 바로 이 스토리가 잡으려는 것
  - [x] 하네스 자신의 스텁 판별: **막힌 상태에서 일부러 소켓을 열어 보고
        실제로 터지는지** 테스트로 고정한다. 아무것도 안 막는 하네스도
        "위반 없음"을 낸다(1.1 교훈 · 1.3 P4 · 2.1·2.2에서 반복 적용)

- [x] **Task 3: 차단 상태 종단 실행 — 같은 코드가 같은 결과를 낸다** (AC: 1)
  - [x] 연결 상태와 차단 상태에서 **동일한 실행 경로**를 돌려 결과를 비교한다.
        비교 대상은 "성공했다"가 아니라 **시뮬레이터 최종 상태·이벤트 로그가
        바이트 단위로 같은가**여야 한다 — "둘 다 성공"은 약한 단언이다
  - [x] 차단 상태에서 `execute_routine`이 **오류 없이** 완료되고,
        `deliver_chunks` 경로를 탔음이 결과에 남는다
  - [x] ⚠️ 이 테스트가 **하네스 없이도 통과하면 의미가 없다** — Task 2의
        하네스가 실제로 활성인 상태에서 도는지 테스트가 스스로 확인할 것

- [x] **Task 4: 차단 사실의 화면 확인** (AC: 2)
  - [x] 데모 실행 중 **차단 상태를 화면에 표시**한다. "연출 의심 차단"이
        AC의 문면이므로, 표시는 **우리 주장이 아니라 관찰**이어야 한다:
        차단 하네스가 활성인지, 그리고 **차단이 실제로 작동함을 그 자리에서
        시연**(일부러 연결 시도 → 실패 확인)
  - [x] ⚠️ **화면 표시의 한계도 같이 적을 것**: 프로세스 내 차단은
        "이 파이썬 프로세스가 못 나간다"이지 "장비가 네트워크에서 끊겼다"가
        아니다. **기내모드는 사람이 눌러야 하고 그건 코드가 증명할 수 없다.**
        발표 시연 절차는 문서로 남기고(Task 6), 화면에는 코드가 아는 것만 적는다
  - [x] 배너 규약 준수 — `docs/CARRIER_INTERFACE.md §4-b`
        (경계마다 한 번, 반복 스트림엔 생략). 새 출력이 생겨도 재발명 금지

- [x] **Task 5: 대비 자료 — 기존 앱 경로** (AC: 3)
  - [x] ⚠️ **이 AC는 코드로 완결되지 않는다.** 실제 ThinQ 앱을 기내모드에서
        돌리는 것은 **사람이 폰으로 하는 일**이다. 우리가 만들 수 있는 것은
        **기록의 틀**이지 결과가 아니다
  - [x] `docs/OFFLINE_COMPARISON.md` 신설 — 두 경로 대비표의 **틀**과
        측정 절차를 정의하고, 우리 경로의 결과는 실측으로 채운다.
        **ThinQ 앱 쪽은 빈칸 + "미측정"으로 남긴다.** 추정치를 적지 않는다(NFR6)
  - [x] 근거 연결: P-1(43.4%) 인용과 이 대비가 같은 주장의 두 면임을 명시.
        VOC가 "서버 죽으면 아무것도 안 됨"을 말했고, 이 표가 그 구조를 보인다
  - [x] ⚠️ 앱 측정을 하게 되면 **캡처·날짜·기기·앱 버전**을 함께 기록할 것.
        기록 없는 비교는 발표에서 반박당한다

- [x] **Task 6: 발표 시연 절차서** (AC: 2, 3)
  - [x] `docs/DEMO_SCRIPT.md` 신설 — 발표장에서 **무엇을 어떤 순서로 보이는가**.
        Epic 2가 발표의 마지막 증거이므로 이 문서가 그 실행 대본이다
  - [x] 포함할 것: 사전 준비(기기·프로필 적재), 시연 순서, **기내모드 전환 시점**,
        화면에서 심사위원이 볼 것, 예상 질문과 답(특히 *"서버 없다면서 DB는?"* —
        발견/처방 트랙 구분), **실패 시 폴백**(BLE 안 뜨면 루프백으로, 단 정직 표기)
  - [x] 말해도 되는 것 / 안 되는 것 표를 `docs/OFFLINE_EVIDENCE.md §2`에서
        가져와 대본에 박아둔다 — 발표 중 즉흥으로 과장하는 것을 막는 장치

- [x] **Task 7: 테스트** (AC: 1, 2, 3)
  - [x] `tests/test_offline_enforcement.py` 신설
  - [x] R5: `deliver_chunks` 왕복, 수신 측 재조립, 기존 `deliver` 경로 보존,
        `reassembled_by` 값 정확성, 2.1·2.2 테스트 무손상
  - [x] 하네스: 차단 활성 시 소켓·DNS·subprocess가 **실제로 터진다**(스텁 판별),
        컨텍스트 종료 후 원상복구, 중첩 사용, 예외 발생 시에도 복구
  - [x] 종단: 연결/차단 두 상태의 **최종 상태·이벤트가 동일**
  - [x] 문서: `OFFLINE_COMPARISON.md`의 ThinQ 칸이 **"미측정"으로 남아 있음**을
        단언한다 — 나중에 추정치로 채워지는 것을 막는 회귀 테스트
  - [x] 회귀 기준선: **258 passed** (`66b3a87`)
...
- RED: `offline_guard` 없음 → collection error (구현 전 실패 확인)
- GREEN 1차: 15/20 — R5·하네스·종단(Task 1~3) 통과, 데모·문서(Task 4~6) 미작성
- GREEN 2차: 20/20 — 데모 `--offline` 플래그, `OFFLINE_COMPARISON.md`,
  `DEMO_SCRIPT.md` 작성 후 전체 통과
- `--offline` 실측: `reassembled_by="receiver"`로 R5가 실제로 풀렸음 확인.
  화면에 "청크가 전송 계층을 거쳐 수신 측에서 재조립됨(무선 구간 구조 반영)"
- 전체 회귀: **278 passed** (258 + 20, 회귀 0)

### 설계 판단 — `OfflineViolation`을 `BaseException`으로

함정 2가 "검토하라"고만 했던 것을 **`BaseException` 파생으로 확정**했다.
근거: 제품 코드는 곳곳에서 `except Exception`으로 fail-closed를 구현한다
(예: `LoopbackTransport.deliver`, `execute_routine`). 위반이 그 `except`에
흡수되면 오류 목록으로 변해 조용히 삼켜지고, 이 스토리 전체가 무의미해진다.
`BaseException`은 일반 `except Exception`에 잡히지 않으므로 위반이 반드시
밖으로 나온다. `pytest.raises`·컨텍스트 매니저 `__exit__` 복구는 정상 동작한다.
`test_violation_not_swallowed_by_product_code`가 이 판단을 고정한다 —
`execute_routine`을 통과하는 위반이 실제로 밖으로 나옴을 단언.

### Completion Notes List

- **Task 1 (R5 해소)**: `LoopbackTransport.deliver_chunks(list[bytes])` 추가 —
  **수신 측이 재조립**한다(무선 구간의 실제 구조). `execute_routine`은
  `deliver_chunks`가 있으면 청크를 그대로 넘기고(`reassembled_by="receiver"`),
  없으면 구형 `deliver(bytes)`로 폴백하며 그 사실을 결과에 남긴다
  (`"sender"`). 2.1 계약 `deliver()`는 보존 — 기존 테스트 무손상.
  경로 혼재는 조용히 넘기지 않고 오류로 거부. 청크 재전송 프로토콜은
  만들지 않음(함정 7, 네 번째 적용).
- **Task 2 (하네스)**: `offline_guard.enforce_offline()` 컨텍스트 매니저 —
  `socket.socket`·`create_connection`·**`getaddrinfo`·`socketpair`**(2.2 리뷰
  Paige 지적)·`urlopen`·`subprocess.Popen`/`run`을 실제 차단. 재진입 안전
  (중첩 카운트), 정상/예외 종료 모두 원상복구. 하네스 스텁 판별 테스트 포함.
- **Task 3 (종단)**: 연결/차단 두 상태의 **최종 상태·이벤트 로그가 동일**함을
  단언("둘 다 성공"보다 강함). 하네스가 실제 활성인지 테스트가 스스로 확인.
- **Task 4 (화면)**: `demo_routine.py --offline` — 차단 시연(소켓 열기 →
  실패 확인)을 **관찰로** 보이고, **한계**("이 프로세스가 못 나간다"까지,
  기내모드는 사람이 누름)를 같이 출력. 배너 규약 유지.
- **Task 5 (대비 자료)**: `docs/OFFLINE_COMPARISON.md` — 우리 경로는 실측,
  **ThinQ 앱은 "미측정" 빈칸**. 측정 절차(앱 버전·기기·날짜·캡처) 정의.
  회귀 테스트가 빈칸이 추정치로 채워지는 것을 막는다.
- **Task 6 (시연 절차서)**: `docs/DEMO_SCRIPT.md` — 시연 순서, 두 층위 차단
  (프로세스/장비) 구분, 예상 질문·답("서버 없다면서 DB는?" 포함),
  말해도 되는 것/안 되는 것 표, 폴백 절차.
- **Task 7 (테스트)**: `tests/test_offline_enforcement.py` 20개.
- 부수: `docs/OFFLINE_EVIDENCE.md`의 "실제 차단 실험은 2.3" 문장을
  "2.3에서 한 걸음 나아갔다(감시→차단)"로 갱신, 남은 한계 명시.

### File List

- `offline_guard.py` — 신규 (오프라인 강제 하네스, 시험 장비)
- `appliance_sim/transports/loopback.py` — 수정 (`deliver_chunks` 추가, `deliver` 보존)
- `home_profile/routine.py` — 수정 (재조립을 수신 측으로, `reassembled_by`)
- `demo_routine.py` — 수정 (`--offline` 플래그, 조건부 정직 표기, 차단 시연)
- `docs/OFFLINE_COMPARISON.md` — 신규 (대비 자료 틀, ThinQ 미측정)
- `docs/DEMO_SCRIPT.md` — 신규 (발표 시연 절차서)
- `docs/OFFLINE_EVIDENCE.md` — 수정 (2.3 진전 반영)
- `tests/test_offline_enforcement.py` — 신규 (20 tests)
- `docs/implementation-artifacts/2-3-offline-enforcement.md` — 본 파일

### Change Log

- 2026-07-22: Story 2.3 구현 완료 — R5 해소(수신 측 재조립), 오프라인 강제
...
- **`ble_bless.py` 존치** (2.1·2.2에서 이월) — 실기기 데모에서 가민 워치가
  **중앙(Central) 역할**이면 Python 주변장치 경로가 불필요하다. 2.3에서 전송
  인터페이스(`deliver_chunks`)를 손댔으나 삭제 결정은 내리지 않았다(함정 7 유지).
  `bless` 자체가 이 환경에서 설치 불가임(2.1 실측)까지 근거가 모였으므로,
  Epic 2 완료(2.4) 시점에 `docs/DECISIONS.md`에 올려 사람이 판단할 것.
