...
| 2026-08-06 | **어젠다 정정 — 지금은 기획단** (daria). ThinQ API A안은 구현 단계 백로그로 이관(선행 조건 에어컨 보유로 해소). 현 단계 실작업 = **산출물 갭 해소**: 특강 요구 목록 대비 전무 4종(PRD&MVP 범위·유스케이스 다이어그램·유스케이스 명세서·메뉴구조도) + 화면설계서 vs UX 계약("미구현 화면 추가 창작 금지") 충돌 1건. **PRD&MVP 범위부터 착수** — 소급 작성임을 문서에 명시(운영 원칙 4) | [PRD_MVP.md](planning-artifacts/PRD_MVP.md) |
...
| 2026-08-06 | **ThinQ API 대응 — 주 전송 경로 채택 불가**. LG가 2024-12 ThinQ API 공식 개방(산출물 특강 p.42). 그러나 흐름이 "사용자→**서버**→ThinQ API→가전"이라 **P-1(클라우드 SPOF)을 그대로 재현** — 주 전송으로 쓰면 온바디 논지가 붕괴한다. 예상 질문("공개됐는데 왜 안 썼나") 답변 확정. **A안 확정**(daria) — 대조군·실증: 실기기를 API로 연결 후 서버를 끊어 죽는 것을 실연하고 옆에서 온바디는 생존. C(주 전송)는 채택 불가, B(보조 경로)는 보류. **A안 효과 정정: TRL은 오르지 않는다** — 실기기에 붙는 것은 문제 쪽이고 해법은 여전히 시뮬레이터이므로 **해법 TRL 4 불변**, 오르는 것은 **POV 증거 강도**(텍스트 43.4% → 실기기 실연). 발표에서 "실기기로 검증했다" 금지 | [PRESENTATION_SPINE §4](PRESENTATION_SPINE.md) |
...
| 2026-08-07 | **마무리 워딩 확정 — "개선했다" → "증명했다"** (daria). 전후 비교 대상이 없으므로 "개선"은 있지도 않은 기준선을 함의한다. 확정문 = **"이 구조가 서버 없이 성립한다는 것을 PoC로 증명했습니다."** 단 "증명"의 **무조건형은 기존 금지어**(OFFLINE_EVIDENCE §2·DEMO_SCRIPT §6의 `❌ "서버 통신이 없음을 증명했다"`)이므로 **범위 한정어 2개**("이 구조가 서버 없이 성립한다는 것" = 대상 한정 / "PoC로" = 단계 한정)를 뺄 수 없다. 하나라도 빠지면 금지어가 된다. 구두 축약 시엔 "증명" 대신 **"보였습니다"**로 낮춘다 | [PRESENTATION_SPINE §7.1](PRESENTATION_SPINE.md)·[DECK_OUTLINE §1 7부](DECK_OUTLINE.md) |
