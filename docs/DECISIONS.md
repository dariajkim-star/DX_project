...
| 2026-08-07 | **TRL 전면 노출 유지 확정** (daria). 노출 강도를 낮추자는 안(정부 R&D 어휘라 채용 심사에서 낯설다)은 기각. 근거 넷: ①TRL을 빼면 성숙도 자백 장표가 "미완성입니다"라는 **변명**으로만 읽힌다 — TRL은 낮은 숫자를 **객관 지표로** 말하게 해주는 장치다 ②트랙별 성숙도 도식이 무너진다(TRL 5 경계선 "시뮬레이터는 못 넘는다" + 도구 5~6 > 제품 3 역전이 눈금 없이는 성립 불가) ③금지어 "TRL 6"이 눈금 없이는 작동하지 않는다 ④"미구현을 미구현이라 표기한다"는 서명은 **측정 가능한 축**이 있어야 주장이 된다. **단 용어 한 줄 정의를 먼저 말하고, 9단계 정의표는 장표가 아니라 주머니에 둔다** | [MATURITY_POSITION §8](MATURITY_POSITION.md)·[DECK_OUTLINE §5](DECK_OUTLINE.md) |
| 2026-08-07 | **분석 도구 본선 승격 — 2026-07-22 "Epic 5 발표 서사 밖" 결정을 뒤집는다** (daria. Amelia 소수의견 채택). 원 결정은 **투자 심사 전제**였고 그 전제에서는 옳았다 — 투자자에게 분석 도구는 제품 이야기가 아니다. **심사자가 채용 담당(인사팀+기술 면접관)으로 바뀌면서 전제가 소멸**했고, 평가 대상이 제품이 아니라 사람이므로 도구는 "어떻게 일하는가"의 직접 증거가 된다(산출물 특강 p.3 "어떤 고민·이슈·노력·개선안"). **배치 = #17, 증명(16) 다음·빈 칸(18) 앞** — 17번에서 계보 게이트를 "내가 만든 장치"로 소개해야 18번의 "게이트가 거부해서 실행이 막혔다"가 변명이 아니라 **작동 결과**로 읽힌다. 순서를 뒤집으면 "못 했지만 봐주세요"가 된다. 말할 것 3가지 = 계보 게이트 · 자기 보고 신뢰도 폐지(출처 날조 3회 → G 감사 점수로 교체) · 97.6점 + **감점 내역 전건**. **금지: TRL 5~6 자랑**("제품은 못 만들고 도구만 만들었네"가 된다). **시스템 분리 표기는 유지** — 올라간 것은 "일하는 방식"이지 제품의 일부가 아니다. 본선 18장 → **19장**, 주머니는 승격분을 빼고 한 겹 더 깊은 것(배점표·감점 전건)만 남긴다 | [SPINE §7.2](PRESENTATION_SPINE.md)·[DECK_OUTLINE §1 6부](DECK_OUTLINE.md) |
